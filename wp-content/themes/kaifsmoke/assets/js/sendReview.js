  ...
  if ($(".reviews_block")) {
    $(".reviewWrite_form .content_send__form").on("click", function () {
      ...
      if (!isValid(name, "[-a-zA-Z-а-яА-Я]+$")) {
        error.push("<p>Некоректное имя. Исправьте пожалуйста</p>");
        name.addClass("error");
      } else {
        error.push();
      ...
      if (!rating) {
        error.push("<p>Вы забыли указать рейтинг. Нам очень важно знать Вашу оценку :)</p>");
      ...
      if (!message.val()) {
        error.push("<p>Кажется, Вы забыли написать нам сообщение. Мы не сможем Вам помочь, если не прочтем его</p>");
