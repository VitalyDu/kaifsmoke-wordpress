...
<h2>
    ...
    echo wp_kses_post($before . sprintf(__('Order #%s', 'woocommerce') . $after . ' от <time datetime="%s">%s</time>', $order->get_order_number(), $order->get_date_created()->format('c'), wc_format_datetime($order->get_date_created())));
    ?>
</h2>
    ...
    <table class="td" cellspacing="0" cellpadding="6"
        ...
        if ($item_totals) {
            ...
            foreach ($item_totals as $total) {
                ...
                //sorry...
                $temp_name = $total['label'];
                if ($temp_name == 'Подытог:')
                ...
                if ($total['label'] == 'Доставка:' && !empty($order->get_shipping_address_1())) {
                    ...
                    if (!empty($posts)) {
                        ...
                        $id = $posts[0]->ID;
                        $address = 'м. ' . get_post_meta($id, 'metro', true) . ', ' . get_post_meta($id, 'address', true) . ', <a href="' . get_post_permalink($id) . '" target="_blank">Как добраться?</a>';
                        ?>
                        <tr>
                            <th class="td" scope="row" colspan="2"
                            ...
                            <td class="td"
                                style="text-align:<?php echo esc_attr($text_align); ?>;"><?= $address ?></td>
                        </tr>
                        <?php
                    }
                }
        ...
        if ($order->get_customer_note() && FALSE) { //скрыто
            ?>
            <tr>
                <th class="td" scope="row" colspan="2"
