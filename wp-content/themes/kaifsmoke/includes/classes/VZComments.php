...
class VZComments
{
    public static function send()
    {
        ...
        if (empty($_POST['data']) || empty($_POST['data']['id'])) {
            $out['error_desc'][] = 'Проверьте правильность введенных данных. Возможно ошибка у нас. Напишите в чат поддержки снизу справа или попробуйте снова :)';
            die(json_encode($out));
        }

        if (empty($_POST['data']['name'])) {
            $out['error_desc'][] = 'Некоректное имя. Исправьте пожалуйста';
            die(json_encode($out));
        }

        if (empty($_POST['data']['rating'])) {
            $out['error_desc'][] = 'Вы забыли указать рейтинг. Нам очень важно знать Вашу оценку :)';
            die(json_encode($out));
        }

        if (empty($_POST['data']['message'])) {
            $out['error_desc'][] = 'Кажется, Вы забыли написать нам сообщение. Мы не сможем Вам помочь, если не прочтем его';
            die(json_encode($out));
        }
