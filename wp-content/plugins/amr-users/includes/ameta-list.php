...
global $amr_refreshed_heading, $totalitems; 	
	...
	if (empty($aopt['list'][$ulist])){ //20210624
...
// figure out what we are doing - searching, filtering -------------------------------------------------------
	...
	$ftypes = amrmeta_get_fieldtypes();  //20211129 needed to finesse date filtering against different raw formats 
...
// if  at initial display not searching or filtering
		...
		// then we are filtering
			...
			foreach ($icols as $cindex => $col) {
				if (!empty ($options[$col]) ) { 
				...
				else { // we are doing some kind of date filter
					if ( !empty($options[$col.'_fromdate']) ) {// has date range
						$from[$col] = $options[$col.'_fromdate'];
						if (isset($ftypes[$col])) { // 20211129 allow for other raw date format types
							if ($ftypes[$col] == 'date_slashes') 
								$from[$col] = str_ireplace('-','/', $from[$col]);
							elseif ($ftypes[$col] == 'date_dots')  
								$from[$col] = str_ireplace('-','.', $from[$col]);
						}
						
					}
					elseif ( !empty($options[$col.'_fromunix']) ) {// has date range in unix 
						$from[$col] = strtotime($options[$col.'_fromunix']);
					}
					elseif ( !empty($options[$col.'_from']) ) {// is this number range? or maybe even text range?
					...
					if ( !empty($options[$col.'_todate']) ) {
						...
						if (isset($ftypes[$col])) { // 20211129 allow for other raw date format types
							if ($ftypes[$col] == 'date_slashes') 
								$to[$col] = str_ireplace('-','/', $to[$col]);
							elseif ($ftypes[$col] == 'date_dots')  
								$to[$col] = str_ireplace('-','.', $to[$col]);
						}
						//echo "<br />looking for dates between ".$from[$col].' and '.$to[$col] ;
					}
