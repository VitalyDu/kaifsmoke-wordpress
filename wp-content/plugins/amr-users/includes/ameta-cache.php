...
/* ---------------------------------------------------------------------- */	
	...
	// note search text has been sanitised
		...
		if (($s[0] == '"') AND ($s[strlen($s) - 1] == '"'))  {  //if there are quotes, it might be want that phrase like an 'and' not 'or'
			$phrase = trim ($s, '"');
			// if only one word, not a phrase, then search for just that word in a field.
			if (stristr($phrase, ' ') === FALSE) { // if there is no space, use the string with quotes
				$likes =  " csvcontent LIKE '%".$s."%' ";
			}
			else {			
			$likes =  " csvcontent LIKE '%".$phrase."%'  "; 
		//	$likes =  ' csvcontent LIKE "%%%s%%"  ';  // search eaxct phrase
			//$args[] = $phrase; //2021112
			}
		}
		else { // more than 1
			$excludelikes = array(); //20211112
			$s = trim($s,' '); // trim spaces, only allow inside a quote
			$s = explode(' ',$s); 
			$likes = '';
			foreach ($s as $i => $word) {
				//if word has a minus, then exclude entries with that word
				if (($word[0] == '-')) {
					$excludelikes[] = "  csvcontent NOT LIKE '%%%s%%' ";
					$exargs[] = trim($word,'-');
				}	
				else {
					$inclikes[$i] = "  csvcontent LIKE '%%%s%%' ";
					$args[] = $word;
				}
			}
			if (!empty($inclikes)) {  // we just excluding?
				if (count($inclikes) > 1) 
					$likes = '('.implode (' OR ', $inclikes).')'; // some folks want an 'AND'
				else 
					$likes = $inclikes[0];
			}
			
			if (!empty($excludelikes)) {
				$exlikes =  '('.implode (' AND ', $excludelikes).')';
				$args = array_merge($args,$exargs);
			}
			if (!empty($likes)) {
				if (!empty($exlikes))  
					$likes .= ' AND '.$exlikes;
			}
			else $likes = $exlikes;		//we had no search just excluding searc	
		}
