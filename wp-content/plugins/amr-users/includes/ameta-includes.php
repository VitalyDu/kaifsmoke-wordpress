...
function amru_new_DateTime_nonfatal ($datetimestring, $tzobj) { // date time create with exception trap to avoid fatal errors
		...
		try {	
			...
			if (current_user_can('manage_options') or current_user_can('manage_user_lists')) {  //20210915 change order to make it clearer that it is a data error
			echo '<p><b>'
			.__('Message to admin only:','amr-users').'</b> '
			.__('You may have some non date data in a date field. ','amr-users').' '
			...
			.'</br >'.__('Please check the content of your date fields, and any fields where you have specified a date format','amr-users').' '
			.'</br >';
			echo 'php message: '.$e->getMessage().'</p>';  // 2017 08 18 get actual message
			}
...
global $amain, $aopt, $amr_current_list;
	...
	if (empty($s)) $s = array(); // deal with dodgy data 20210715
...
global $aopt;
	...
	if (empty($aopt['list'][$list])) {
		amr_flag_error ('Error: no settings for list '.$list);
		return (array());  //20210715 catch bad settings
	}
	if (empty($aopt['list'][$list]['selected'])) {
		amr_flag_error ('Error: no fields selected for list '.$list);
		return (array());
	}
...
function amr_users_dropdown ($choices, $current) { // does the options of the select
	...
	foreach ($choices as $opt => $value){	
		echo '<option value="'.esc_attr($opt).'"';  //22020327
