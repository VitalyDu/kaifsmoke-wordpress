...
global $amain, $aopt;
	...
	//	require_once(ABSPATH . 'wp-admin/includes/admin.php');
		if (empty($_FILES) ) {
			amr_users_message(__('No file specified','amr-users'));  //20210715
			return;
		}
		if (empty($_FILES['importfile'])) {
			amr_users_message(__('Problem in upload','amr-users')); //20210715
			return;
		}
		...
		if (!empty($errors)) {
			amr_users_message(__('There was an error uploading your file.','amr-users').' '.implode(', ',$errors['upload_error']) );
			return;
		} 
		
		if (empty($uploaded['file'])) {
			amr_users_message(__('WP did not upload file.','amr-users').' '.implode(', ',$uploaded)); // could be file mime type is not allowed 20160207
			//202100715  Dump the upload return variable to see what wp says.  This may not be correct, had example where it was a strange character in the file.
			return;
		}
