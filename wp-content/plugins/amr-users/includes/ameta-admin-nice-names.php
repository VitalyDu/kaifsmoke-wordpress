...
function amr_check_raw_data_for_field($fld) { //20211129
	global $wpdb;
	echo '<h1>Look at the raw data for '.$fld.'</h1>';
	// onlty works if in user meta
	$sql = $wpdb->prepare( "
        SELECT DISTINCT meta_value FROM {$wpdb->usermeta}
        WHERE meta_key = '%1s'
		LIMIT 50",$fld);
		
	echo '<p>SQL_QUERY: <br/>'.$sql.'</p>';
	$results = $wpdb->get_results( $sql );
	
	if (!empty($results)) {
		foreach ($results as $row) {
			echo '<br />'.($row->meta_value);
			}
	}
			...
			if (is_array($_POST['nn'])) {
				foreach ($_POST['nn'] as $i => $v) {
					if (empty($v)) $amr_nicenames[$i] = '';
						...
						if	($s = sanitize_text_field($v))  //20220326
							$amr_nicenames[$i] = $s;
						else { 
...
function ameta_list_nicenames_for_input($nicenames) {
	...
	/* get the standard names and then the  meta names  */
		...
		foreach ($nicenames as $i => $v ) {
			...
			echo '<td><input type="text" size="40" id="nn'.$i.'"  name="nn['.$i.']"  value= "'.esc_attr($v).'" />'; //20220327
			echo '</td>';
			...
			echo '</select>';
			if (!empty($orig_mk[$i]) ){ // 2021129 only for fiedls in usermeta for now
				echo '<a class="check" href="'
		//	.wp_nonce_url(add_query_arg('check_field',$i,admin_url('admin.php?page=page=ameta-admin-nice-names.php')),'amr-meta')
			.add_query_arg('check_field',$orig_mk[$i],admin_url('admin.php?page=ameta-admin-nice-names.php'))
			.'">check raw data</a>';
			}
			echo '</td>';			
// end 20170112	
...
function amr_meta_nice_names_page() {
	...
	if (isset($_REQUEST['check_field'])) {
		$fld = sanitize_text_field($_REQUEST['check_field']);
		amr_check_raw_data_for_field($fld);
		return;
	}
