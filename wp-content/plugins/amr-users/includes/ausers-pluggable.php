...
// have they asked for a link?	
	...
	$v = apply_filters('amr_users_before_format_value', $v, $i, $u);	 //20220322 to allow for things like wpml translation
...
// end fix icols and cols
		...
		if (!empty($adminoptions['show_headings'])) { //admin always has
			...
			if (empty($linessaved)) $uclass = ' empty'; //20210617
			else $uclass= '';	
		}		
	...
	//
		...
		elseif (is_admin() and amr_users_can_edit('filtering')) {	// in admin  and plus function available etc					
			...
			if (!empty($adminoptions['show_headings']) and (!empty($linessaved) and (empty($adminoptions['start_empty'] )))) 	//20210617
	...
	//
			...
			if (!empty($adminoptions['show_totals'])) {     //20210317  add ability to show totals
				$total_text	= PHP_EOL.'<div id="user_totals" class='.$uclass.'><p>'  //20210617
			...
			$html = amr_manage_headings_submit() //will only show if relevant
				...
				.$total_text
				.$pagetext
				.PHP_EOL.'<div id="userslist'.$ulist.'" class="userslist'.$uclass.'"><!-- amr users list-->'.PHP_EOL
				.$ahtm['table']		
				.$caption
				.$hhtml
				.PHP_EOL
