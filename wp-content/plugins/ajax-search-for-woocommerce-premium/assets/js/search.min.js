...
(function (factory) {
                ...
                if (tokens[i] && tokens[i].length >= 1) {
                  ...
                  if (token.length > 0) {
                    if (token.trim().length === 1 && tokens[i] !== last) {
                      var pattern =
                        "((\\s|^)" +
                        utils.escapeRegExChars(token.trim()) +
                        "\\s)";
                    } else if (
                      token.trim().length === 1 &&
                      tokens[i] === last
                      ...
                      var pattern =
                        "((\\s|^)" + utils.escapeRegExChars(token.trim()) + ")";
                    } else {
                      var pattern =
                        "(" + utils.escapeRegExChars(token.trim()) + ")";
                    }
  ...
  DgwtWcasAutocompleteSearch.prototype = {
    ...
    createContainers: function (type) {
      ...
      if ($("." + options.containerClass).length == 0) {
        that.suggestionsContainer = $(
          DgwtWcasAutocompleteSearch.utils.createNode(options.containerClass)
        );
      ...
      if (that.canShowDetailsBox()) {
        if ($("." + options.containerDetailsClass).length == 0) {
          that.detailsContainer = $(
            DgwtWcasAutocompleteSearch.utils.createNode(
              options.containerDetailsClass
            )
          );
    ...
    registerEventsSearchBar: function () {
      ...
      that.el
        .closest("." + that.options.formClass)
    ...
    registerEventsSuggestions: function () {
      ...
      $(document).on(
        ...
        function () {
          ...
          var selector =
            '.dgwt-wcas-suggestion[data-index="' + currentIndex + '"]';

          var timeOffset = that.canShowDetailsBox() ? 100 : 1;

          if (that.selectedIndex != currentIndex) {
            utils.mouseHoverDebounce(
              function () {
                if (that.selectedIndex !== currentIndex) {
      ...
      $("." + that.options.containerClass).on(
        "mousedown.autocomplete",
        function (e) {
          var that = utils.getActiveInstance();
    ...
    registerEventsDetailsPanel: function () {
      ...
      if (
        !that.canShowDetailsBox() ||
        detailsContainer.hasClass("js-dgwt-wcas-initialized")
      ) {
        return;
      }
      ...
      // Mark cursor position for onBlur event
      $("." + that.options.containerDetailsClass).on(
        "mousedown.autocomplete",
        function (e) {
          var that = utils.getActiveInstance();
          that.isMouseDownOnSearchElements = true;
    ...
    registerIconHandler: function () {
      ...
      if (
        $(".js-dgwt-wcas-initialized").length == 0 &&
        $(".js-dgwt-wcas-search-icon-handler").length > 0
      ) {
        $(document).on("click.autocomplete", function (event) {
          if ($(".dgwt-wcas-layout-icon-open").length) {
            ...
            if (
              !(
                $target.closest("." + that.options.searchFormClass).length >
                  0 ||
                $target.closest("." + that.options.containerClass).length > 0 ||
                $target.closest("." + that.options.containerDetailsClass)
                  .length > 0
              )
            ) {
    ...
    registerEventsFixedMenu: function () {
      ...
      $(window).on("scroll.autocomplete", function () {
        if (
          ...
          that.elementOrParentIsFixed(that.getFormWrapper())
        ) {
    ...
    setOptions: function (suppliedOptions) {
      ...
      $suggestionsContainer.css({
        "max-height": !that.canShowDetailsBox()
    ...
    close: function () {
      ...
      that.el
        .closest("." + that.options.searchFormClass)
        .find("." + that.options.searchInputClass)
        .val("")
        .focus();
    },
    disable: function () {
    ...
    fixPositionDetailsBox: function () {
      ...
      offset.left =
        leftOffset +
        Math.round($containerSuggestions.width() + borderCorrection);
      ...
      if ($form.width() >= maxWidth) {
        ...
        if (that.options.isRtl === true) {
          $containerSuggestions.css(
            "left",
            leftOffset +
              Math.round($containerDetails.width() + borderCorrection) +
              "px"
          );
      ...
      if (windowWidth < cOffset.left + cDWidth) {
        ...
        $containerDetails.css(
          "left",
          Math.round(
            parseFloat($containerSuggestions.css("left").replace("px", ""))
          ) -
            $containerDetails.outerWidth() +
            "px"
        );
      }
    ...
    automaticAlignment: function () {
      ...
      that.autoAligmentprocess = setInterval(function () {
        ...
        if (that.options.showDetailsPanel) {
          var innerDetailsHeight = $detailsWrapp
            .find(".dgwt-wcas-details-inner")
            .height();
    ...
    getFormOffset: function () {
      ...
      var orientation = that.options.orientation,
        ...
        styles = { top: 94, left: offset.left };
        ...
        utils.debounce(function () {
          ...
          that.currentRequest = $.ajax(ajaxSettings)
            .done(function (data) {
              ...
              that.options.onSearchComplete.call(
                that.element,
                q,
                result.suggestions
              );
      ...
      if (result != null) {
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            var currentObjectID = that.prepareSuggestionObjectID(
              that.suggestions[that.selectedIndex]
            );
    ...
    updatePrices: function (noAjax) {
      ...
      if (!noAjax && productsToLoad.length > 0) {
        var data = {
          action:
            typeof dgwt_wcas.action_get_prices == "undefined"
                ...
                if (that.suggestions.length > 0) {
                  for (j = 0; j < that.suggestions.length; j++) {
                    if (
                      ...
                      (that.suggestions[j].type == "product" ||
                        that.suggestions[j].type == "product_variation") &&
    ...
    updatePrice: function (index, price) {
      ...
      if (typeof that.suggestions[index] != "undefined") {
        ...
        var $price = $(".dgwt-wcas-suggestions-wrapp").find(
          '[data-index="' + index + '"] .dgwt-wcas-sp'
        );
    ...
    detailsPanelSetScene: function (objectID) {
      var that = this,
        ...
        $el = $containerDetails.find(
          '.dgwt-wcas-details-inner[data-object="' + objectHash + '"]'
        );
    ...
    detailsPanelAddToScene: function (objectID) {
      ...
      if (typeof object != "undefined" && typeof object.html == "string") {
        html = object.html.replace(
          "<div ",
          '<div data-object="' + objectHash + '" '
        );
      }
      ...
      if (!that.isMobileMode()) {
        $(document).mouseup(function (e) {
          ...
          var $container = that.getSuggestionsContainer(),
            $containerDetails = that.getDetailsContainer(),
            outsideForm = !(
              $(e.target).closest("." + that.options.searchFormClass).length >
                0 || $(e.target).hasClass(that.options.searchFormClass)
            ),
            outsideContainer = !(
              $(e.target).closest("." + that.options.containerClass).length >
                0 || $(e.target).hasClass(that.options.containerClass)
            );
          ...
          if (!that.canShowDetailsBox()) {
            ...
            var outsidecontainerDetails = !(
              $(e.target).closest("." + that.options.containerDetailsClass)
                .length > 0 ||
              $(e.target).hasClass(that.options.containerDetailsClass)
            );
    ...
    suggest: function () {
      ...
      $.each(that.suggestions, function (i, suggestion) {
        ...
        if (
          ...
          if (suggestion.taxonomy === "product_cat") {
            ...
            if (!options.showHeadings) {
              prepend +=
                ...
                dgwt_wcas.labels["tax_" + suggestion.taxonomy] +
                "</span>";
            }
            if (
              typeof suggestion.breadcrumbs != "undefined" &&
            ...
            options.isPremium &&
            suggestion.taxonomy === options.taxonomyBrands
          ) {
            ...
            suggestion.value =
              dgwt_wcas.labels.show_more + " (" + suggestion.total + ")";
            highlight = false;
          } else if (options.showHeadings && suggestion.type === "headline") {
            classes +=
              " js-dgwt-wcas-suggestion-headline dgwt-wcas-suggestion-headline";
            if (
              typeof dgwt_wcas.labels[suggestion.value + "_plu"] != "undefined"
            ) {
          ...
          // Image
          if (
            typeof suggestion.image_src != "undefined" &&
          ...
          if (suggestion.type === "vendor") {
            html +=
              ...
              prepend +
              formatResult(suggestion.value, value, highlight, options) +
              append +
              "</span>";
            ...
            if (suggestion.shop_city) {
              html +=
                ...
                formatResult(suggestion.shop_city, value, true, options) +
                "</span>";
            }
            ...
            if (typeof suggestion.desc != "undefined" && suggestion.desc) {
              html +=
                ...
                formatResult(suggestion.desc, value, true, options) +
                "</span>";
            }
          } else {
            html +=
          ...
          if (
            ...
            typeof suggestion.thumb_html != "undefined"
          ) {
            is_img = true;
          }
          ...
          // One suggestion HTML
          dataAttrs +=
            typeof suggestion.post_id != "undefined"
              ? 'data-post-id="' + suggestion.post_id + '" '
              : "";
          dataAttrs +=
            typeof suggestion.taxonomy != "undefined"
              ? 'data-taxonomy="' + suggestion.taxonomy + '" '
              : "";
          dataAttrs +=
            typeof suggestion.term_id != "undefined"
              ? 'data-term-id="' + suggestion.term_id + '" '
              : "";
          html +=
            '<a href="' +
            url +
          ...
          if (is_img) {
            html +=
              '<span class="dgwt-wcas-si">' + suggestion.thumb_html + "</span>";
          }
          ...
          if (
            typeof suggestion.title_before != "undefined" &&
            suggestion.title_before
          ) {
          ...
          html +=
            '<span class="dgwt-wcas-st-title">' +
            formatResult(suggestion.value, value, true, options) +
            parent +
            "</span>";

          // Custom content after title (3rd party)
          if (
            typeof suggestion.title_after != "undefined" &&
            suggestion.title_after
          ) {
          ...
          // SKU
          if (
            options.showSKU === true &&
            typeof suggestion.sku != "undefined" &&
            suggestion.sku.length > 0
          ) {
            html +=
              '<span class="dgwt-wcas-sku">(' +
              dgwt_wcas.labels.sku_label +
              " " +
              formatResult(suggestion.sku, value, true, options) +
              ")</span>";
          }
          ...
          if (
            ...
            typeof suggestion.desc != "undefined" &&
            suggestion.desc
          ) {
            html +=
              '<span class="dgwt-wcas-sd">' +
          ...
          // Vendor
          if (
            options.showProductVendor === true &&
            typeof suggestion.vendor != "undefined" &&
            suggestion.vendor
          ) {
            var vendorBody =
            ...
            if (
              typeof suggestion.vendor_url != "undefined" &&
              suggestion.vendor_url
            ) {
          ...
          if (
            typeof suggestion.content_after != "undefined" &&
            suggestion.content_after
          ) {
          ...
          if (showPrice) {
            html +=
              '<span class="dgwt-wcas-sp">' + suggestion.price + "</span>";
          }
      ...
      if (that.options.orientation === "top") {
        that
          ...
          .addClass(that.classes.suggestionsContainerOrientTop);
    ...
    positionPreloader: function ($formWrapper) {
      var $submit =
        typeof $formWrapper == "object"
          ? $formWrapper.find(".dgwt-wcas-search-submit")
    ...
    preloader: function (action, place, cssClass) {
      var that = this,
        ...
        cssClasses =
          cssClass == null ? defaultClass : defaultClass + " " + cssClass;
    ...
    onSelect: function (index) {
      ...
      if (that.currentValue !== that.el.val() && !that.options.preserveInput) {
        that.el.val(
          that.currentValue.replace(/(<([^>]+)>)/gi, " ").replace(/\s\s+/g, " ")
        );
      }
    ...
    disableOverlayMobile: function ($overlayWrap) {
      ...
      var $clonedForm = $(".js-dgwt-wcas-om-bar").find(
        "." + that.options.searchFormClass
      );
    ...
    showCloseButton: function () {
      var that = this,
        iconBody =
          typeof dgwt_wcas.close_icon != "undefined"
            ? dgwt_wcas.close_icon
            : "",
        $actionsEl = that
          .getFormWrapper()
          .find("." + that.options.preloaderClass);
        ...
        try {
          if (typeof gtag !== "undefined") {
            gtag("event", "page_view", {
              page_path:
                "/?s=" + encodeURI(label) + "&post_type=product&dgwt_wcas=1",
            });
          } else if (gaObj !== false) {
  ...
  (function () {
    ...
    $(document).ready(function () {
      ...
      window.dgwt_wcas.config = {
        ...
        dynamicPrices:
          typeof dgwt_wcas.dynamic_prices != "undefined" &&
          dgwt_wcas.dynamic_prices
            ? true
            : false,
        ...
        appendTo:
          typeof dgwt_wcas.suggestions_wrapper != "undefined"
            ? dgwt_wcas.suggestions_wrapper
            : "body",
        showProductVendor:
          typeof dgwt_wcas.show_product_vendor != "undefined" &&
          dgwt_wcas.show_product_vendor
            ? true
            : false,
        disableHits:
          typeof dgwt_wcas.disable_hits != "undefined" && dgwt_wcas.disable_hits
            ? true
            : false,
        disableSubmit:
          typeof dgwt_wcas.disable_submit != "undefined" &&
          dgwt_wcas.disable_submit
            ? true
            : false,
      };
    ...
    $(window).on("load", function () {
      ...
      if (
        ...
        typeof window.elementorFrontend.documentsManager.documents !=
          "undefined"
      ) {
        $.each(
