...
class DetailsBox {
	...
	private function getPostTypeDetails__premium_only( $postID ) {
		...
		$vars = array(
			...
			'imageSrc'    => $post->getThumbnailSrc( 'medium' ),
			'imageSrcset' => $responsiveImages ? $post->getThumbnailSrcset( 'medium' ) : '',
			'imageSizes'  => $responsiveImages ? $post->getThumbnailSizes( 'medium' ) : '',
		...
		ob_start();
		$file = DGWT_WCAS_DIR . 'partials/details-panel/' . $postType . '.php';
