...
class Indexer {
	 ...
	 */
	public function insert( $postID, $directVariations = false ) {
		global $wpdb;
		$success = false;
		...
		if ( $postType === 'product' ) {
			...
			if ( $this->product->isType( 'variable' ) && DGWT_WCAS()->settings->getOption( 'search_in_product_sku' ) === 'on' ) {
				$this->enqueueOrIndexVariations( $directVariations );
			}
		...
		do_action( 'dgwt/wcas/readable_index/after_insert', $dataFiltered, $postID, $postType, $success, $data );
	 ...
	 * @param bool $directVariations Index variations directly
	 */
	private function enqueueOrIndexVariations( $directVariations = false ) {
		...
		if ( $directVariations ) {
			...
			$totalVariations = Builder::getInfo( 'total_variations_for_indexing' );
	...
	public function update( $postID, $directVariations = false ) {
		$this->delete( $postID );
		$this->insert( $postID, $directVariations );
	}
