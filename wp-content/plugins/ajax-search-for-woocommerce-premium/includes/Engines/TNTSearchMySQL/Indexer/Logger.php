...
class Logger {
	const SOURCE = 'fibosearch-indexer';
	 ...
	 * @param string $source
	 */
	public static function removeLogs( $source = Logger::SOURCE ) {
		self::maybeInit();
	...
	public static function handleThrowableError( $e, $prefix = '' ) {
		...
		if ( $errorType !== false ) {
			...
			$nonCriticalErrors = Builder::getInfo( 'non_critical_errors' );
	...
	public static function getLastEmergencyLog() {
		...
		$files = self::$wcLogHandlerFile->get_log_files();
		foreach ( $files as $handle => $filename ) {
			if ( strpos( $handle, Logger::SOURCE ) !== false ) {
	...
	/**
	 * Get error code by it's message
	 *
	 * @param $message
	 *
	 * @return bool|string
	...
	public static function getErrorCode( $message ) {
		preg_match_all( '/\[1203] User (.*) already has more than \'max_user_connections\' active connections/mi', $message, $matches, PREG_SET_ORDER, 0 );
		if ( isset( $matches[0] ) ) {
			return '[Error code: 003]';
		}
		preg_match_all( '/\[1040] Too many connections/mi', $message, $matches, PREG_SET_ORDER, 0 );
		if ( isset( $matches[0] ) ) {
