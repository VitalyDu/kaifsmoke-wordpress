...
class SettingsAPI {
	...
	function callback_checkbox( $args ) {
		...
		$html .= sprintf( '<input type="checkbox" class="checkbox" id="%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s %4$s />', $this->name, $args['id'], checked( $value, 'on', false ), $args['input_data'] );
	...
	function callback_filters_rules_plug( $args ) {
		...
		<div id="dgwt-wcas-settings-filters-rules">
			...
			<button class="button button-secondary"><?php _e( 'Add new rule', 'ajax-search-for-woocommerce' ); ?></button>
		</div>
		<?php
