...
class Install {
	...
	private static function maybeUpgradeOptions__premium_only() {
		$settingsVersion = (int) get_option( 'dgwt_wcas_settings_version_pro', 0 );

		if ( $settingsVersion < 1 ) {
			if ( (int) get_option( 'dgwt_wcas_settings_version', 0 ) === 0 ) {
				self::upgradeOptionsTo1();
			}
			update_option( 'dgwt_wcas_settings_version_pro', 1 );
			DGWT_WCAS()->settings->clearCache();
		}
	}
		...
		if ( ! defined( 'IFRAME_REQUEST' ) ) {
			...
			if ( dgoraAsfwFs()->is__premium_only() ) {
				...
				if ( get_option( 'dgwt_wcas_version_pro' ) != DGWT_WCAS_VERSION ) {
					...
					self::maybeUpgradeOptions__premium_only();
